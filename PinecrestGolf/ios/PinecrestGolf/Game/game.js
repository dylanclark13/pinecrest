...
function beginCharge(){
  if(phase==='ready')nativeCheckpoint().catch(e=>window.webkit.messageHandlers.nativeError.postMessage(e.message));if(phase==='accuracy'){commitStrike();return;}
...
const TUTORIAL=[
 ...
 {chapter:'Getting started',title:'Choose your challenge',copy:'Eight courses offer 144 holes. Score 90 or lower in a full round at Atlantic Links to open Blackwater Dunes. Earn Bronze on each challenge to open the next. Silver requires 81 and Gold requires 72. Choose front nine, back nine, or a full round. Willow Park has wider fairways and slower greens; Pinecrest adds tighter lines, Granite Highlands adds elevation, and Atlantic Links has strong wind and fast greens.',cue:'RELAXED → CLUB → TOUR → CHAMPIONSHIP',tip:'Create your look in My golfer. Start with nine holes while learning.',question:'Which course gives you the most forgiving first round?',options:['Atlantic Links','Willow Park','Granite Highlands'],answer:1,feedback:'Willow Park offers the widest timing window and more forgiving conditions.'},
...
function animate(now){
  const dt=Math.min((now-last)/1000,.05);last=now;if(nativePaused){requestAnimationFrame(animate);return;}if(replay){if(!document.hidden)playReplay(dt);requestAnimationFrame(animate);return;}const paused=isModal()||document.hidden||homeOpen;
  if(!paused){
