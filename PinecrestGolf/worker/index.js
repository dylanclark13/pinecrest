...
import {medal,unlockedCourses,dailyChallenge} from './challenges.js';
...
async function courseProgress(env,user){
 ...
 return {courseBest:best,courseMedals:Object.fromEntries(Object.entries(best).map(([c,n])=>[c,medal(n)])),unlockedCourses:unlockedCourses(best)};
}
...
async function currentRound(env,user){const row=await db(env).prepare("SELECT id,course,mode,daily_day,next_hole,end_hole,status FROM rounds WHERE user_id=? AND status='active' ORDER BY created_at DESC LIMIT 1").bind(user).first();if(!row)return null;const saved=await db(env).prepare('SELECT hole,strokes FROM round_holes WHERE round_id=? ORDER BY hole').bind(row.id).all();return {...row,daily:row.daily_day?dailyChallenge(row.daily_day):null,scores:saved.results};}
...
async function handle(request,env,user,account=null){
 ...
 try{
  ...
  if(path==='/api/daily'&&request.method==='GET'){
   const daily=dailyChallenge(),reward=await db(env).prepare('SELECT tokens FROM daily_rewards WHERE user_id=? AND day=?').bind(user,daily.day).first();
  ...
  if(path==='/api/daily'&&request.method==='POST'){
   const daily=dailyChallenge(),id=crypto.randomUUID();await ensurePlayer(env,user);
   await db(env).batch([db(env).prepare("UPDATE rounds SET status='closed' WHERE user_id=? AND status='active'").bind(user),db(env).prepare('INSERT INTO rounds(id,user_id,course,mode,daily_day,next_hole,end_hole,status,created_at) VALUES(?,?,?,?,?,?,?,?,?)').bind(id,user,daily.course,'daily',daily.day,daily.start,daily.end,'active',Date.now())]);
   return json({round:await currentRound(env,user),profile:await profile(env,user)});
  }
  ...
  if(match&&request.method==='POST'){
   ...
   const dailyStatements=isDaily&&done?[
    db(env).prepare('INSERT OR IGNORE INTO daily_rewards(user_id,day,tokens,round_id,receipt) SELECT ?,?,?,?,? WHERE EXISTS(SELECT 1 FROM round_holes WHERE round_id=? AND hole=? AND receipt=?)').bind(user,round.daily_day,dailyTokens,id,receipt,id,b.hole,receipt),
    db(env).prepare('UPDATE players SET tokens=tokens+? WHERE user_id=? AND EXISTS(SELECT 1 FROM daily_rewards WHERE user_id=? AND day=? AND receipt=?)').bind(dailyTokens,user,user,round.daily_day,receipt)
   ]:[];
   await db(env).batch([
    db(env).prepare('INSERT OR IGNORE INTO round_holes(round_id,hole,strokes,xp,tokens,receipt,putts,fairway,gir) VALUES(?,?,?,?,?,?,?,?,?)').bind(id,b.hole,b.strokes,0,earned,receipt,metrics?.putts??null,metrics?.fairway??null,metrics?.gir??null),
    db(env).prepare('UPDATE players SET tokens=tokens+?,holes=holes+?,rounds=rounds+? WHERE user_id=? AND EXISTS(SELECT 1 FROM round_holes WHERE round_id=? AND hole=? AND receipt=?)').bind(earned,isDaily?0:1,done&&!isDaily?1:0,user,id,b.hole,receipt),
    db(env).prepare('UPDATE rounds SET next_hole=?,status=? WHERE id=? AND user_id=? AND EXISTS(SELECT 1 FROM round_holes WHERE round_id=? AND hole=? AND receipt=?)').bind(b.hole+1,done?'complete':'active',id,user,id,b.hole,receipt),...dailyStatements
   ]);
   const dailyAward=dailyTokens?await db(env).prepare('SELECT tokens FROM daily_rewards WHERE user_id=? AND day=? AND receipt=?').bind(user,round.daily_day,receipt).first():null;
   return json({profile:await profile(env,user),earned:earned+(dailyAward?.tokens||0),dailyReward:dailyAward?.tokens||0,roundBonus:bonus,round:await currentRound(env,user)});
  }
