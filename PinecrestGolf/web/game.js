...
function commitStrike(){
  ...
  phase='downswing';downswingTime=0;actor.phase='downswing';actor.progress=0;actor.power=power;actor.club=activeClub();actor.angle=angle;actor.pure=pure;actor.perfect=perfect;actor.cinematic=perfect&&!window.matchMedia('(prefers-reduced-motion: reduce)').matches;updateUI();
}
function impact(){
...
function applyRound(r){roundId=r.id;courseIndex=r.course;course=COURSES[courseIndex];HOLES=course.holes;roundMode=r.mode;roundStart=roundMode==='back'?9:0;roundEnd=roundMode==='front'?8:17;scores=Array(18).fill(null);for(const s of r.scores||[])scores[s.hole]=s.strokes;hasRound=true;closeHome();startHole(r.next_hole);showRoundNotice();}
